...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    auditLog = pgTable("audit_log", {
      ...
      // equipmentCode for evidence items
      tableName: varchar("table_name").notNull(),
      // 'evidence_items', 'equipment_groups', etc.
      metadata: jsonb("metadata"),
      // Contains the diff/changes made
...
var init_dynamic_ai_config = __esm({
  "server/dynamic-ai-config.ts"() {
    ...
    DynamicAIConfig = class {
       ...
       */
      static async logAIUsage(auditLog2) {
        try {
          console.log(`[Dynamic AI Config] Audit: ${auditLog2.usedProvider} used for incident ${auditLog2.incidentID}`);
          console.log(JSON.stringify(auditLog2, null, 2));
        } catch (error) {
...
async function registerRoutes(app3) {
  ...
  app3.post("/api/equipment-groups-commented-out", async (req, res) => {
...
if (!encryptionKey || encryptionKey.length < 32) {
  console.error("\u{1F6A8} PROTOCOL VIOLATION: AI_KEY_ENCRYPTION_SECRET missing or too short (must be 32+ characters)");
